  ...
  return (
    <div style={{ background: 'var(--surface-card)', border: '1px solid var(--surface-border)', borderRadius: 'var(--r-lg)', padding: 'var(--s4)', marginBottom: 'var(--s5)' }}>
      <div style={FILTER_GRID}>
        <div>
          <label style={LABEL} htmlFor="hist-activo">Activo / Lote <span aria-hidden style={{ color: 'var(--sem-error)' }}>*</span></label>
          {activosError ? (
            <input id="hist-activo" type="number" style={INPUT} placeholder="ID de activo…" value={value.idActivo} onChange={(e) => set('idActivo', e.target.value)} />
          ) : (
        ...
        <div>
          <label style={LABEL} htmlFor="hist-desde">Desde <span aria-hidden style={{ color: 'var(--sem-error)' }}>*</span></label>
        ...
        <div>
          <label style={LABEL} htmlFor="hist-hasta">Hasta <span aria-hidden style={{ color: 'var(--sem-error)' }}>*</span></label>
          <input id="hist-hasta" type="date" style={INPUT} value={value.fecha_fin} onChange={(e) => set('fecha_fin', e.target.value)} />
        </div>

        <div>
          <label style={LABEL} htmlFor="hist-nivel">Nivel de riesgo</label>
        ...
        <div>
          <label style={LABEL} htmlFor="hist-pat">Patología</label>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 'var(--s2)', fontSize: '13px', color: 'var(--text-secondary)', cursor: 'pointer', paddingBottom: 'var(--s2)' }}>
            <input type="checkbox" checked={value.incluir_alertas} onChange={(e) => set('incluir_alertas', e.target.checked)} style={{ accentColor: 'var(--brand-500)', width: 16, height: 16 }} />
            Incluir alertas
          </label>
        </div>
      </div>

      {errFechas && <p role="alert" style={{ margin: '0 0 var(--s2)', fontSize: '12px', color: 'var(--sem-error)' }}>La fecha inicial no puede ser posterior a la final.</p>}
      ...
      <div style={{ display: 'flex', gap: 'var(--s2)', justifyContent: 'flex-end' }}>
        ...
        <Button variant="primary" size="sm" loading={loading} disabled={!value.idActivo || !!errFechas} onClick={onConsultar}>
          <Search size={15} aria-hidden style={{ marginRight: 'var(--s1)' }} /> Consultar
        </Button>
      </div>
    </div>
  );
