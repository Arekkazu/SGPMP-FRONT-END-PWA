      ...
      {loading || !modelo ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--s5)' }}>
          <div style={{ display: 'flex', gap: 'var(--s2)', flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {modelo.esta_produccion && <Pill tono="success">En producción</Pill>}
          </div>

          {saveError && <Alert variant={saveError.status === 403 ? 'warning' : 'error'} title="Operación rechazada" description={saveError.message} />}

          {/* Métricas de validación */}
          <div>
            <h3 style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 var(--s3)' }}>Métricas de validación</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: 'var(--s3)' }}>
              <Metrica label="F1 global" valor={pct(modelo.f1_score)} destacado />
              <Metrica label="Recall riesgo alto" valor={pct(modelo.recall_clase_riesgo_alto)} destacado />
              <Metrica label="Precisión" valor={pct(modelo.precision_modelo)} />
              <Metrica label="Accuracy" valor={pct(modelo.accuracy)} />
              <Metrica label="ROC-AUC" valor={pct(modelo.roc_auc_score)} />
            </div>
          </div>

          {modelo.matriz_confusion && modelo.matriz_confusion.length > 0 && (
            <div>
              <h3 style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 var(--s3)' }}>Matriz de confusión</h3>
          ...
          {modelo.detalle_validacion && (
            <Alert variant="warning" title="Detalle de validación" description={modelo.detalle_validacion} />
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 'var(--s3)', fontSize: '12px', borderTop: '1px solid var(--surface-border)', paddingTop: 'var(--s4)' }}>
            ...
            <div><span style={{ color: 'var(--text-muted)' }}>Tamaño: </span>{modelo.tamanio_artefacto_bytes ? `${(modelo.tamanio_artefacto_bytes / 1_048_576).toFixed(1)} MB` : '—'}</div>
            <div style={{ gridColumn: '1 / -1', fontFamily: 'var(--font-mono)', wordBreak: 'break-all' }}>
              <span style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-sans, inherit)' }}>Hash SHA-256: </span>{modelo.hash_artefacto_sha256 ?? '—'}
            </div>
          </div>

          {/* Notas de validación clínica */}
          <div style={{ borderTop: '1px solid var(--surface-border)', paddingTop: 'var(--s4)' }}>
            <h3 style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 var(--s2)' }}>Notas de validación clínica</h3>
            {puedeEditar ? (
              <>
                <textarea
                  value={notas}
                  onChange={(e) => setNotas(e.target.value)}
                  placeholder="Registra la validación clínica de esta versión antes de activarla en producción…"
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 'var(--s2)' }}>
                  <Button variant="secondary" size="sm" loading={saving} disabled={!online || !notas.trim() || notas.trim() === (modelo.notas_validacion ?? '').trim()} onClick={() => onGuardarNotas(notas.trim())}>
                    <Save size={15} aria-hidden style={{ marginRight: 'var(--s1)' }} /> Guardar notas
                  </Button>
                </div>
              </>
            ) : (
          ...
          {modelo.estado_version === 'APROBADO' && puedeEjecutar && (
            <div style={{ borderTop: '1px solid var(--surface-border)', paddingTop: 'var(--s4)' }}>
              {!tieneNotas && (
                <Alert variant="info" title="Notas requeridas" description="Registra las notas de validación clínica antes de activar esta versión en producción." style={{ marginBottom: 'var(--s3)' }} />
              )}
              <Button variant="primary" disabled={!puedeActivar} onClick={onSolicitarActivar}>
                <Rocket size={16} aria-hidden style={{ marginRight: 'var(--s1)' }} /> Activar en producción
              </Button>
            </div>
          )}
        </div>
