...
export function VinculacionDetalleModal({ vinc, puedeGestionar, online, onResolver, onCorregir, onClose }: Props) {
  ...
  return (
          ...
          <div style={{ display: 'flex', gap: 'var(--s2)' }}>
            {puedeResolver && (
              <Button variant="primary" size="sm" disabled={disabled} title={disabled ? 'Sin permiso o sin conexión' : 'Resolver'} onClick={onResolver}>
                <GitFork size={15} aria-hidden style={{ marginRight: 'var(--s1)' }} />Resolver
              </Button>
            )}
            {puedeCorregir && (
              <Button variant="danger" size="sm" disabled={disabled} title={disabled ? 'Sin permiso o sin conexión' : 'Corregir'} onClick={onCorregir}>
                <Pencil size={15} aria-hidden style={{ marginRight: 'var(--s1)' }} />Corregir
              </Button>
            )}
          </div>
        ) : undefined
    ...
    >
      ...
      <Fila label="Activo biológico">{vinc.id_activo_biologico != null ? `#${vinc.id_activo_biologico}` : '—'}</Fila>
      <Fila label="Modelo de manejo">{vinc.modelo_manejo || '—'}</Fila>
      <Fila label="Infraestructura">#{vinc.id_infraestructura}</Fila>
      <Fila label="Inicio validez">{fechaHora(vinc.fecha_inicio_vinculacion)}</Fila>
      <Fila label="Fin validez">{fechaHora(vinc.fecha_fin_vinculacion)}</Fila>
      <Fila label="Creada">{fechaHora(vinc.fecha_creacion)}</Fila>
      {vinc.id_usuario != null && <Fila label="Usuario">#{vinc.id_usuario}</Fila>}
      ...
      <div style={{ marginTop: 'var(--s5)' }}>
        <h3 style={{ display: 'flex', alignItems: 'center', gap: 'var(--s2)', fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 var(--s2)' }}>
          <ShieldCheck size={16} aria-hidden /> Trazabilidad NIC 41
        </h3>
        <Fila label="Vinculación reemplazada">{vinc.id_vinculacion_reemplazada != null ? `#${vinc.id_vinculacion_reemplazada}` : '—'}</Fila>
        <Fila label="Motivo de corrección">{vinc.motivo_correccion ?? '—'}</Fila>
      </div>
    </ModalShell>
  );
}
