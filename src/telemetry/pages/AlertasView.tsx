      ...
      <div style={{ padding: 'var(--s5) var(--s7)', borderBottom: '1px solid var(--surface-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 'var(--s4)', flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ display: 'flex', alignItems: 'center', gap: 'var(--s2)', fontSize: '20px', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>
            ...
            Alertas Ambientales y Sanitarias
          </h1>
        ...
        <Button variant="ghost" size="sm" onClick={() => { cargar(build(paginacion.pagina)); cargarResumen(); }} aria-label="Recargar">
          <RefreshCw size={15} aria-hidden />
        </Button>
      </div>

      <div style={{ padding: 'var(--s7)' }}>
        {!online && <Alert variant="warning" title="Sin conexión" description="Mostrando alertas cacheadas. La gestión está deshabilitada." style={{ marginBottom: 'var(--s4)' }} />}
        {fromCache && online && <Alert variant="info" title="Datos desde caché" description="No se pudo conectar; se muestran las últimas alertas disponibles." style={{ marginBottom: 'var(--s4)' }} />}
